...
{"id":51,"first_name":"Hello","last_name":"Baine","email":"sbaine1d@intel.com","shirt_size":"M","company_name":"Gabtype","donation":171},
{"id":52,"first_name":"Goodbye","last_name":"MacGiolla Pheadair","email":"amacgiollapheadair1b@xing.com","shirt_size":"2XL","company_name":"Kwimbee","donation":214},
];

// ==== Challenge 1: Use .forEach() ==== map is so much better here
// The event director needs both the first and last names of each runner for their running bibs.  Combine both the first and last names into a new array called fullName. 
let fullName = runners.map(x => `${x.first_name} ${x.last_name}`);
...
let largeShirts = runners.filter(x=> x.shirt_size.toUpperCase() == 'L')
...
// Problem 1 - print every ones name in alphabetic order last name first
let out = runners.map(x=> `${x.last_name}, ${x.first_name}`).sort();
console.log(out);

// Problem 2 - only pick people out that are related (have the same last name)
let samelastname = runners.filter((x,i,a) => 
{
    var flag = false;
    for (let j=0; j<a.length; j++){
        flag = a[j].last_name.toUpperCase() == x.last_name.toUpperCase() && i != j;
        if(flag)break;
    };
    return flag;
}).sort((a,b)=> a.last_name > b.last_name ? 1 : -1);
console.log(samelastname);
// Problem 3 - find the average donation
let averagedonation = runners.reduce((total, x) => {return total + x.donation},0)/runners.length;
